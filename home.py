...
def abrir_home(janela, frame_atual):
    ...
    titulo_home = ctk.CTkLabel(frame_atual, text='Backup Manager (Home)', font=('Helvetica', 22), fg_color='#111530', text_color='white')
    ...
    botao_config = ctk.CTkButton(frame_atual, image=engrenagem_img, text='', fg_color='#111530', hover_color='#A9A9A9', width=60, command=lambda: abrir_config(janela, frame_atual), bg_color='#111530')
    ...
    logoff = ctk.CTkButton(frame_atual, image=sair_img, text='', width=60, fg_color='#111530', hover_color='#A9A9A9', command=lambda: abrir_login(janela, frame_atual), bg_color='#111530')
    logoff.place(relx=0.025, rely=0.02)

    fzr_bkp = ctk.CTkButton(frame_atual, text='Backups', fg_color='#0d1b2a', hover_color='#0d1b2a', command=lambda: abrir_backups(janela, frame_atual), width=80)
    fzr_bkp.place(relx=0.5, rely=0.3, anchor='n')

    fzr_bkp = ctk.CTkButton(frame_atual, text='Contas De Dados', fg_color='#0d1b2a', hover_color='#0d1b2a', command=lambda: backup_dados(janela, frame_atual), width=80)
